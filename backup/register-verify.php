	...
	if(isset($_POST['submit'])) {
		...
		if ($username == "" || $email == "" || $password != $cPassword) {
			...
			if ($num_rows > 0) {
				...
				$query ="INSERT INTO member (Username_member, Password_member, credit, phone_member, line, status_approve, role, start_date, end_date, ref, date_remain, profile_img, line_img, 	nickname, email, verify_token, salt_password) VALUES ('$username', '$hashedPassword' , '0', '$phonenumber', '', '0', 'user', '', '', '', '', '', '', '$nickname', '$email', '$token', '$salt')";
				if ($queryresult = mysqli_query($objCon,$query)) {
					...
					$mail->Body = "
						<div class=\"block-confirm\" style=\"width:320px; margin: 0 auto;text-align: center;background-color: #f5f5f5;box-shadow: 0px 0px 6px 0px rgba(192,192,192,0.8);padding: 20px 10px;\">
							<img src=\"http://35.240.131.230/ufabet/testphpmail/images/logo-de6d8af1.png\" width=\"320\" height=\"auto\" alt=\"\" style=\"margin-bottom: 20px;\">
							<div class=\"title\" style=\"font-weight: 700;margin-bottom: 30px;\">กรุณากดปุ่มยืนยันอีเมล์ เพื่อเข้าสู่ระบบ</div>
							<div class=\"content\">
								<a style=\"text-decoration: none; color: #fff;background-color: #15AAF5; font-weight: 700;font-size: 18px;padding: 10px 20px 10px 20px; border-radius: 3px;\"href='http://35.240.131.230/ufabet-secondary/confirm.php?email=$email&token=$token'>ยืนยันอีเมล์</a>
							</div>
						</div>
					";
					if ($mail->send()) {
